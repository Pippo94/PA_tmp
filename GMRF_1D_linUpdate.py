...
N = 10         # number of training/measurement points.
...
for i in range(len(X_training)):
    ...
    mu_GF = np.dot(Ks.T, np.linalg.solve(Lk.T, np.linalg.solve(Lk, Y_training[0:i+1])))  # GF expectation
    ...
    mu_MRF = np.dot(Qinv, np.dot(M.T, np.dot(np.linalg.inv(sig*np.eye(i+1) + np.dot(M, np.dot(Qinv, M.T))), Y_training[0:i+1])))
